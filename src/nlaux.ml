module KMP = struct
  ...
  let make_pattern p =
    ...
    if l = 0 then invalid_arg "Nlaux.KMP.make_pattern";
    let rex =
      Nlstring_str.regexp (Nlstring_str.quote (String.make 1 p.[0])) in
    ...
    and run_regexp pos =
      ...
      let pos' =
        try
           ...
           *)
          let p, _ =
            Nlstring_str.search_forward rex s pos in (* FIXME: no ~len *)
          p
        with
            Not_found -> endpos
      in
  ...
  let find_pattern pat ?(pos=0) ?len s =
    ...
    if pos < 0 || endpos > String.length s || pos > endpos then
      invalid_arg "Nlaux.KMP.find_pattern";
    ...
    ref
      (fun (src:int array) srcpos dest destpos len ->
         ...
         if (len < 0 || srcpos < 0 ||
           ...
           invalid_arg "Nlaux.ArrayAux.int_blit";
  ...
  let int_series_ref =
    ref
      (fun src srcpos dst dstpos len n ->
         ...
         then
           invalid_arg "Nlaux.ArrayAux.int_series";
