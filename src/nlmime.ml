...
object
  ...
  method content_type : 
           unit -> (string * (string * Nlmimestring.s_param)list)
...
let drop_ws s =
   ...
   * the new string
   *)
  match Nlstring_str.string_match drop_ws_re s 0 with
      None -> ""
    | Some r -> Nlstring_str.matched_group r 1 s
;;
...
object (self)
  ...
  method private immutable() =
    raise (Immutable "Nlmime.basic_mime_header");
  ...
  method content_type() =
    Nlmimestring.scan_mime_type_ep (self#field "content-type") []
  method content_disposition() =
    Nlmimestring.scan_mime_type_ep (self#field "content-disposition") []
...
object (self)
  ...
  method open_value_wr() =
    ...
    if ro then self#immutable();
    ro <- ro';
    let b = Nlbuffer.create 60 in
    new output_netbuffer ~onclose:(fun () -> value <- Nlbuffer.contents b) b;

  method private immutable() =
    raise (Immutable "Nlmime.memory_mime_body");

  method private finalized() =
    failwith "Nlmime.memory_mime_body: object is finalized";
end ;;
...
object (self)
  ...
  method private immutable() =
    raise (Immutable "Nlmime.file_mime_body");
  ...
  method private finalized() =
    failwith "Nlmime.file_mime_body: object is finalized";
end ;;
...
object(self)
  ...
  method ro = true
  method set_value _ = raise (Immutable "Nlmime.complement_mime_body_ro");
  method open_value_wr _ = raise (Immutable "Nlmime.complement_mime_body_ro");
end ;;
...
let read_mime_header ?(unfold=false) ?(strip=true) ?ro stream =
  let h = Nlmimestring.read_header ~downcase:false ~unfold ~strip stream in
...
let decode_mime_body hdr =
  ...
  match encoding with
	...
	(fun s -> 
	   new output_filter 
	     (new Nlencoding.Base64.decoding_pipe 
	      ~url_variant:false ~accept_spaces:true ()) s)
    | "quoted-printable" ->
	(fun s ->
	   new output_filter
	     (new Nlencoding.QuotedPrintable.decoding_pipe()) s)
    | _ ->
	failwith "Nlmime.decode_mime_body: Unknown Content-transfer-encoding"
;;
...
let encode_mime_body ?(crlf = true) hdr =
  ...
  match encoding with
	...
	(fun s -> 
	   new output_filter 
	     (new Nlencoding.Base64.encoding_pipe 
	       ~linelength:76 ~crlf ()) s)
    | "quoted-printable" ->
	(fun s ->
	   new output_filter
	     (new Nlencoding.QuotedPrintable.encoding_pipe ~crlf ()) s)
    | _ ->
	failwith "Nlmime.encode_mime_body: Unknown Content-transfer-encoding"
;;
...
let rec write_mime_message_int ?(wr_header = true) ?(wr_body = true) ?(nr = 0) 
  ...
  let mk_boundary parts =
    ...
    Nlmimestring.create_boundary ~random:data ~nr ()
  in
  ...
  match cbody with
	...
	if wr_header then
	  Nlmimestring.write_header ~eol ~soft_eol:eol outch hdr#fields;
    ...
    | `Parts_ag(cte,parts) ->
	if parts = [] then
	  failwith "Nlmime.write_mime_message: Cannot write multipart message with empty list of parts";
	...
	let boundary =
	  try
		...
		| ex ->
		    failwith ("Nlmime.write_mime_message: Cannot parse content-type field: " ^ Printexc.to_string ex)
	    in  
	    if String.length ctype < 10 || String.sub ctype 0 10 <> "multipart/"
	    then 
	      failwith "Nlmime.write_mime_message: The content type of a multipart message must be 'multipart/*'";
	    try
	      let b = List.assoc "boundary" params in   (* or Not_found *)
	      Nlmimestring.param_value b
	    with
		Not_found ->
