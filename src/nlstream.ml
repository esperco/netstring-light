...
object
  inherit Nlchannels.in_obj_channel
  method block_size : int
  method window : Nlbuffer.t
...
object(self)
  ...
  method really_input buf pos len =
    if s_closed then raise Nlchannels.Closed_channel;
    let rec read p =
  ...
  method input_line () =
    ...
    let rec find_eol() =
      try
        Nlbuffer.index_from s_netbuf 0 '\n'    (* or Not_found *)
      with
          Not_found ->
...
object (self)
  ...
  inherit input_methods (Nlbuffer.create block_size)
  ...
  method private debug msg =
    prerr_endline (msg ^ ": s_pos=" ^ string_of_int s_pos ^
                   ...
                   " buflen=" ^ string_of_int (Nlbuffer.length s_netbuf) ^
    ...
    if not s_at_eof then begin
      ...
      let m =
        match s_maxlength with
            None   -> s_blocksize
          | Some l -> min (l - s_pos - Nlbuffer.length s_netbuf) s_blocksize
      in
      assert(m >= 0);
      (* Try to read m bytes: *)
  ...
  method want n =
    if s_closed then raise Nlchannels.Closed_channel;
    while not s_at_eof && Nlbuffer.length s_netbuf < n do
  ...
  method skip len =
    ...
    let rec read len =
      if len > 0 then begin
        let k = min (Nlbuffer.length s_netbuf) len in
        Nlbuffer.delete s_netbuf 0 k;
        s_pos <- s_pos + k;
        self # want_minimum();    (* may raise Buffer_underrun *)
  ...
  method input buf pos len =
    ...
    let len' = min len (Nlbuffer.length s_netbuf) in
    Nlbuffer.blit s_netbuf 0 buf pos len';
    Nlbuffer.delete s_netbuf 0 len';
    s_pos <- s_pos + len';
    ( try
        self # want_minimum();  (* may raise Buffer_underrun *)
      with
...
object(self)
  ...
  initializer
    (match delimiter with
         Some "" -> invalid_arg "new Nlstream.sub_stream";
       | Some d  -> s_del <- Some(d, Nlaux.KMP.make_pattern d)
       | None    -> s_del <- None
    );
    (match s_len with
         Some l -> if l<0 then invalid_arg "new Nlstream.sub_stream";
       | None   -> ()
    );
    try
      self # want_minimum()
  ...
  method private compute_winlen() =
    ...
    begin match s_del with
        None ->
          ...
          s_at_eof <- weof;
      | Some(d,pat) ->
          let p = Nlaux.KMP.find_pattern
                    pat ~len:wlen (Nlbuffer.unsafe_buffer w) in
